...
io.on('connection', (socket) => {
  ...
  socket.on('send-chat-message', async (data) => {
    ...
    // ALWAYS filter profanity (not optional anymore)
    const filteredMessage = filterProfanity(data.message);
    
    const chatMessage = {
      id: uuidv4(),
      username: socket.username || 'Guest',
      message: filteredMessage.substring(0, 200), // Max 200 chars
      timestamp: Date.now()
    };
...
function startGameLoop(gameId) {
  ...
  const interval = setInterval(async () => {
    ...
    // Broadcast game state (ONLY dynamic data, not buildings/chests!)
    io.to(gameId).emit('game-update', {
      ...
      triviaStations: game.triviaStations
      // NO buildings, NO chests - they're static!
    });
...
function filterProfanity(text) {
  ...
  const badWords = [
    ...
    'crap', 'piss', 'dick', 'cock', 'pussy', 'cunt', 'asshole',
    'motherfucker', 'bullshit', 'fag', 'faggot', 'retard', 'nigger',
    'nigga', 'slut', 'whore', 'douche', 'douchebag', 'jackass',
    'fck', 'fuk', 'f4ck', 'sh1t', 'a$$', 'a55', 'b1tch', 'b!tch',
    'azz', 'arse', 'nazi', 'hitler', 'kkk'
  ];
  ...
  badWords.forEach(word => {
    ...
    filtered = filtered.replace(regex, () => '*'.repeat(Math.max(word.length, 4)));
  });
...
function containsProfanity(text) {
  ...
  const badWords = [
    ...
    'asshole', 'motherfucker', 'bullshit', 'fag', 'faggot', 'retard',
    'nigger', 'nigga', 'slut', 'whore', 'nazi', 'hitler'
  ];
