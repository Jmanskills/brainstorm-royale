...
io.on('connection', (socket) => {
  ...
  socket.on('player-input', (data) => {
    const gameId = playerGames.get(socket.id);
    if (!gameId) {
      console.log('❌ No gameId for socket', socket.id);
      return;
    }
    
    const game = games.get(gameId);
    if (!game || game.state !== 'playing') {
      console.log('❌ No game or game not playing');
      return;
    }
    
    const player = game.players.get(socket.id);
    if (!player || !player.isAlive) {
      console.log('❌ No player or player dead');
      return;
    }
    
    console.log('✅ Processing input from', player.username, 'data:', data);
    ...
    console.log('📍 Movement calculated: dx=', dx, 'dy=', dy);
    ...
    console.log('🏃 Position updated from (', oldX, oldY, ') to (', player.x, player.y, ')');
